...
public class IndirectHeap {
    ...
    public static void initialise(int size, float defaultKey) {
        IndirectHeap.defaultKey = defaultKey;
    ...
    private void bubbleDown(int index) {
        int leftChild = IndirectHeap.leftChild(index);
        int rightChild = IndirectHeap.rightChild(index);
     ...
     * Runtime: O(lgn)
     */
    public bool decreaseKey(int outIndex, float newKey) {
    ...
    public int popMinIndex() {
        if (heapSize == 0)
        {
            throw new AccessViolationException("Indirect Heap is empty!");
        }
        else if (heapSize == 1)
        {
            int s = getOut(0);
